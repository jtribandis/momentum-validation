...
EVENT_FIELDS = ['event_id','evaluation_window','permaticker','historical_ticker','company_name','cik',
                ...
                'last_trade_date','event_type','source_row_ids','policy_target_branch','evidence_required',
...
def main() -> int:
    ...
    def raw_rows(p, ltd, et):
        """Join raw ACTIONS by ACTUAL event date + classification. Returns ALL matches (never r0)."""
        rows = con.execute(f"""SELECT action, date, value, contraticker, contraname, ticker, "name"
            FROM ac WHERE permaticker={p} AND action IN {TERMINAL_ACTIONS}
            AND date BETWEEN DATE '{ltd}' - INTERVAL 45 DAY AND DATE '{ltd}' + INTERVAL 45 DAY
            ORDER BY date, action""").fetchall()
        want = {'ACQUIRED':'acquisitionby','MERGER_NONSURVIVOR':'mergerfrom',
                'BANKRUPTCY_LIQUIDATION':'bankruptcyliquidation','DELISTED_OTHER':'delisted',
                'REGULATORY_DELISTING':'regulatorydelisting','VOLUNTARY_DELISTING':'voluntarydelisting'}.get(et)
        # dedupe identical vendor rows by (action, date): duplicates otherwise mint duplicate
        # exposure_ids for one economic event (F-021)
        seen, out = set(), []
        for r in rows:
            if r[0] != want: continue
            k = (r[0], str(r[1]))
    ...
    def build(exposures, window_tag, is_clone):
        ...
        for e in exposures:
            ...
            rr = raw_rows(p, ltd, et)
            for r in (rr or [(None, None, None, None, None, None, None)]):
